...
auto Mem::findSig(const char* sig) -> uintptr_t {
...
auto Mem::findSigInMod(const char* sig, const char* mod) -> uintptr_t {
    const char* pattern = sig;
	uintptr_t firstMatch = 0;
	static const uintptr_t rangeStart = (uintptr_t)GetModuleHandleA(mod);
	static MODULEINFO miModInfo;
	static bool init = false;
	if (!init) {
		init = true;
